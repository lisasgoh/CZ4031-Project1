    ...
    if (data_file.is_open()) {
        ...
        if (data_file.is_open()) {
            ...
            while (getline(data_file, copied)) {
                ...
                // Copy the data into the block of records - Copy bytes from
                // 'copied' to 'record', untill we encounter tabspace
                strcpy(record.tconst,
                       copied.substr(0, copied.find('\t')).c_str());
                ...
                // pointer arithmetic
                void *rcdAdr =
                    (uchar *)get<0>(data_record) + get<1>(data_record);
                memcpy(rcdAdr, &record, sizeof(record));
            }
            ...
            cout << "1. Size of Database: " << memory_pool.getPoolSize()
                 << "\n";
            cout << "2. Size of One block: " << memory_pool.getBlockSize()
                 << "B\n";
        ...
        for (records_iterator = data.begin(); records_iterator != data.end();
            ...
            void *recordAddress =
                (uchar *)memory_block.at(blockAddress) + offset;
            ...
            keys_struct key;
            key.key_value = num;
            key.add_vect.push_back((uchar *)memory_block.at(blockAddress) +
                                   offset);

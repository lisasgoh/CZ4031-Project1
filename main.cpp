...
int main() {
  cout << "\n------------ Welcome to the Database Management System "
          "------------\n"
          "This is a simulation of the database management system, to demonstrate storage and indexing of a database, designed by Group 8.\n"
       << "\n";
  ...
  if (data_file.is_open()) {
    ...
    while (getline(data_file, copied)) {
      ...
      // Copy the data into the block of records - Copy bytes from 'copied' to 'record', untill we encounter tabspace
      strcpy(record.tconst, copied.substr(0, copied.find('\t')).c_str());

      stringstream line_stream(copied);
      getline(line_stream, tempLine, '\t');

      // Write the blocks of records into disk, with the attributes in the dataset

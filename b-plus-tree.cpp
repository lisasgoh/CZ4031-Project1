        ...
        for(int i = 0; i < cursor->size; i++){
            ...
            if(cursor->key[i].key_value == x){
                ...
                if (flag == true) {
                    ...
                    cout << "num nodes accessed: " << nodesAccessed << endl;
                    cout <<"Number of "<< x <<": "<< cursor->key[i].add_vect.size() << "\n";
                    ...
                    for (int j = 0; j < cursor->key[i].add_vect.size(); ++j) {
                        void *recordAddress = cursor->key[i].add_vect[j];
                        printf("Record address: ");
                        cout << (Record *)cursor->key[i].add_vect[j] << endl;
                        cout << "block number = " << ((uint)recordAddress - (uint)startAddress)/blockSize<<endl;
                        //printf("\n");
                        //printf("tconst: ");
                        //cout << (*(Record *) cursor->key[i].add_vect[j]).tconst << "\n";
                        totalAverageRating += (*(Record *) cursor->key[i].add_vect[j]).averageRating;
                    }
                    cout << "Average Rating: " << totalAverageRating / cursor->key[i].add_vect.size() << endl;
  ...
  if (root == NULL) {
    ...
    if (leftSibling >= 0) {
      ...
      if (leftNode->size >= (MAX + 1) / 2 + 1) {
        ...
        cursor->key[0].add_vect[0] <<" from left sibling of leaf node\n";
        return;
      }
    }
    if (rightSibling <= parent->size) // check if right sibling exist
    {
      ...
      if (rightNode->size >= (MAX + 1) / 2 + 1) {
        ...
        cursor->key[cursor->size-1].add_vect[0] <<" from right sibling of leaf node\n";
        return;
      }
    }

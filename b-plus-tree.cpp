        ...
        for (int i = 0; i < cursor->size; i++) {
            ...
            if (cursor->key[i].key_value == x) {
                ...
                if (flag == true) {
                    ...
                    cout << "tconst | record addr | data block no | data block addr" << endl;
                    for (int j = 0; j < cursor->key[i].add_vect.size(); ++j) {

                        uint dataBlockNo = ((uint)(Record *)cursor->key[i].add_vect[j] - (uint)startAddress)/ blockSize;
                        // get tconst value
                        cout << (*(Record *)cursor->key[i].add_vect[j]).tconst << " | "
                             ...
                             << dataBlockNo << " | "
                             // get block addr
                             << (Record *)((uint)startAddress + (dataBlockNo * blockSize))
                             << endl;
                    ...
                    cout << "Average Rating: "
                         << totalAverageRating / cursor->key[i].add_vect.size() << endl;

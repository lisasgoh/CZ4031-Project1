...
class BPTree
{
    ...
    void insertInternal(keys_struct x, Node* cursor, Node* child)
    {
        ...
        else
        {
            ...
            //make space for new key (shift everyth upwards)
            ...
            if(cursor == root)
            {
                ...
                newRoot->key[0] = cursor->key[cursor->size];//This should be the smallest element of the right??
...
public:
    ...
    Node * search(float lowerBound, float upperBound) {
        if (root == nullptr) {
            ...
            while (!cursor->IS_LEAF) {
                   ...
                   for(int i = 0; i < cursor->size; i++)
                {
                    ...
                    // If the searched key is larger than all the keys in this node, go to the right pointer's node.
                    if(i == cursor->size - 1)
                    {
                        // if (printer == true) {
                        //     for (int j = 0; j < cursor->size; j++) {
            ...
            while (!stop) {
                ...
                for (i = 0; i < cursor->size; i++) {
                    ...
                    if (cursor->keys[i] >= lowerBound && cursor->keys[i] <= upperBound) {
                        cout << "Number of Data Blocks: %d", cursor->key[i].add_vect.size());
                        printf("\n");
                ...
                if (cursor->ptr[cursor->size] != nullptr && cursor->keys[i] != upperBound) {

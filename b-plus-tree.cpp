...
void BPTree::insertInternal(keys_struct x, Node *cursor, Node *child) {
  ...
  if (cursor->size < MAX) {
    ...
    } else { // New key that should be added in the parent node.
      keys_struct newKey = getNewKey(newInternal);
...
int BPTree::removeInternal(keys_struct x, Node *cursor, Node *child,
  ...
  if (rightSibling <= parent->size) {
    ...
    if (rightNode->size >= (MAX + 1) / 2) {
      ...
      // cursor. pos represents the index of the pointer that points to the
      // cursor node. Shift leftmost key of right sibling to cursor's last key.
  ...
  if (leftSibling >= 0) {
    ...
    int numNodesMerged =
        removeInternal(parent->key[leftSibling], parent, cursor, leftNode);
  ...
  // Try to merge right sibling.
    ...
    int numNodesMerged = removeInternal(parent->key[rightSibling - 1], parent,
                                        rightNode, cursor);
    return 1 + numNodesMerged;
  }
}
...
int BPTree::remove(keys_struct x) {
  ...
  if (root == nullptr) {
    ...
    while (x.key_value > cursor->key[pos].key_value)
      pos++;
    if (x.key_value !=
        cursor->key[pos].key_value) // if key does not exist in that leaf node
    {
  ...
  if (root == nullptr) {
    ...
    while (cursor->key[i].key_value <= upperBoundKey) {
      for (int j = 0; j < cursor->key[i].add_vect.size(); j++) {
        ...
        intptr_t dataBlockNo = ((intptr_t)(Record *)cursor->key[i].add_vect[j] -
      ...
      if (i == cursor->size - 1 && cursor->ptr[cursor->size] != nullptr &&
          cursor->key[i].key_value <= upperBoundKey) {
        cursor = cursor->ptr[cursor->size];
        i = 0;
        continue;
      }
...
void BPTree::searchSingle(float key) {
  ...
  if (root == nullptr) {
    ...
    cout << "Average Rating: " << totalRating / cursor->key[i].add_vect.size()
         << endl;
    cout << "Number of data blocks accessed: " << s.size() << endl;
  }
}
